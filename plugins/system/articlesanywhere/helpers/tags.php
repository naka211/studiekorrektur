...
class plgSystemArticlesAnywhereHelperTags
{
	...
	private function passIfStatement(&$if, &$article, &$count = 0, $first = 0, $last = 0)
	{
		...
		$php = str_replace('$article->empty', ($count ? 0 : 1), $php);
	...
	public function replaceTags(&$text, &$matches, &$article, &$count)
	{
		...
		foreach ($matches as $match)
		{
			$string = $this->processTag($match['1'], $count);
			if ($string === false)
			{
				continue;
			}
	...
	public function processTag($data, &$count)
	{
		...
		return $this->processTagByType($tag, $extra, $count);
	}

	public function processTagByType($tag, $extra, &$count)
	{
		switch (true)
		{
			// Link closing tag
